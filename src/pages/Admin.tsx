...
export default function AdminPage() {
  ...
  }, [dispatch, siteSettings.disable_registration]);
  ...
  const canAccessTab = useCallback((key: string) => {
    if (isRegistrationDisabled && (key === 'reviews' || key === 'manage-clients')) {
      return false;
    }
  ...
  }, [hasPermission, isAdmin, role, isRegistrationDisabled]);
  ...
  useEffect(() => {
    ...
    const fallback = role === 'staff'
      ...
      : (role === 'manager'
          ? 'analytics'
          : (isRegistrationDisabled ? 'appointments' : 'manage-clients'));
    navigate(TAB_PATHS[fallback] || TAB_PATHS.analytics, { replace: true });
  }, [activeTab, role, user, navigate, canAccessTab, isRegistrationDisabled]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col font-sans">
      ...
      {/* Top Header Bar */}
      <header className="h-14 md:h-16 bg-[#121212]/90 backdrop-blur-md border-b border-gray-800/80 flex items-center justify-between px-4 md:px-6 shrink-0 z-30 shadow-2xl">
        <div className="flex items-center gap-3 md:gap-4">
          ...
          <img src={logo} alt="1625 Autolab" className="h-7 md:h-8.5 w-auto object-contain" referrerPolicy="no-referrer" />
          <span className="hidden sm:block text-gray-700 text-lg select-none">/</span>
          <span className="hidden sm:inline-flex items-center gap-2 px-2.5 py-1 bg-brand-orange/10 border border-brand-orange/30 text-brand-orange text-[10px] font-mono font-bold uppercase tracking-widest rounded-full">
            Admin Console
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <a
            href="/"
            className="flex items-center gap-1.5 text-xs font-mono font-bold uppercase tracking-wider text-gray-400 hover:text-brand-orange transition-colors"
          >
          ...
          <button
            ...
            className="relative group w-9 h-9 rounded-full bg-brand-orange/20 border-2 border-brand-orange/40 hover:border-brand-orange flex items-center justify-center overflow-hidden shrink-0 transition-all shadow-md cursor-pointer"
          >
            <img
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-mono font-bold uppercase tracking-wider text-gray-400 hover:text-red-400 transition-colors cursor-pointer"
          >
      ...
      <div className="relative flex flex-1 min-h-0 overflow-hidden">
        ...
        <aside className={`
          ...
          flex flex-col bg-gradient-to-b from-[#111111] via-[#0d0d0d] to-[#080808] border-r border-gray-800/80 flex-shrink-0
        ...
        `}>
          ...
          <div className={`border-b border-gray-800/80 ${collapsed ? 'p-3' : 'p-4 md:p-5'}`}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-brand-orange/20 border-2 border-brand-orange/40 flex items-center justify-center shrink-0 overflow-hidden shadow-md">
                <img
                  src={user.avatar_url || fallbackAvatar}
                  alt="User avatar"
                  className="w-full h-full object-cover"
              ...
              {!collapsed && (
                <div className="min-w-0">
                  ...
                  <p className="text-brand-orange font-mono text-[10px] uppercase font-bold tracking-wider mt-0.5">{user.role}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <nav className={`${collapsed ? 'p-2' : 'p-3'} space-y-1.5 flex-grow overflow-y-auto font-sans`}>
            {navItems.map((item) => {
              if (item.isGroup && item.children) {
                ...
                return (
                  <div key={item.key} className="mb-2">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-all duration-150 rounded-lg border border-transparent text-gray-400 hover:text-white hover:bg-gray-800/60 hover:border-gray-700/60 cursor-pointer ${collapsed ? 'justify-center' : ''}`}
                    >
                      <div className="flex items-center gap-3">
                        <GroupIcon className="w-4 h-4 shrink-0 text-brand-orange" />
                        {!collapsed && <span>{item.label}</span>}
                      </div>
                      {!collapsed && (
                        <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isOpen ? 'rotate-180 text-brand-orange' : 'text-gray-500'}`} />
                      )}
                    </button>

                    {/* Group Sub-items */}
                    {!collapsed && isOpen && (
                      <div className="mt-1 space-y-1 pl-3 border-l border-gray-800/80 ml-4">
                        {visibleChildren.map(child => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-mono font-bold uppercase tracking-wider transition-all duration-150 rounded-lg relative cursor-pointer ${
                                isActive
                                  ? 'text-brand-orange bg-brand-orange/15 border border-brand-orange/30 shadow-[0_0_12px_rgba(249,115,22,0.15)] font-black'
                                  : 'text-gray-400 hover:text-white hover:bg-gray-800/50 border border-transparent hover:border-gray-800'
                              }`}
                            >
                              <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-brand-orange rounded-r-full transition-opacity ${isActive ? 'opacity-100' : 'opacity-0'}`} />
              ...
              return (
                <div key={item.key} className="mb-1">
                  <button
                    ...
                    title={collapsed ? item.label : undefined}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs font-mono font-bold uppercase tracking-wider transition-all duration-150 rounded-lg relative cursor-pointer ${
                      isActive
                        ? 'text-brand-orange bg-brand-orange/15 border border-brand-orange/30 shadow-[0_0_12px_rgba(249,115,22,0.15)] font-black'
                        : 'text-gray-400 hover:text-white hover:bg-gray-800/50 border border-transparent hover:border-gray-800'
                    } ${collapsed ? 'justify-center' : ''}`}
                  >
                    <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-brand-orange rounded-r-full transition-opacity ${isActive ? 'opacity-100' : 'opacity-0'}`} />
                    <FlatIcon className="w-4 h-4 shrink-0 text-brand-orange" />
                    {!collapsed && <span>{item.label}</span>}
                  </button>
                </div>
              );
          ...
          {/* Desktop Collapse & Mobile Sign Out Footer */}
          <div className="border-t border-gray-800/80 p-3 flex items-center gap-2 bg-[#0c0c0c] mt-auto shrink-0">
            <button
              onClick={() => setCollapsed(v => !v)}
              title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
              className="hidden md:flex items-center justify-center w-full px-3 py-2.5 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800/60 transition-colors font-mono text-xs uppercase font-bold tracking-wider cursor-pointer"
            >
              {collapsed
                ? <ChevronRight className="w-4 h-4 text-brand-orange" />
                : <><ChevronLeft className="w-4 h-4 mr-2 text-brand-orange" /><span>Collapse Sidebar</span></>
              }
            </button>
            ...
            <button
              ...
              onClick={handleLogout}
              title="Sign Out"
              className="md:hidden w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs font-mono font-bold uppercase tracking-wider text-gray-400 hover:text-red-400 transition-colors cursor-pointer"
            >
        ...
        <main className={`flex-1 min-w-0 min-h-0 ${isChatbotTab ? 'p-0 overflow-hidden' : 'p-4 sm:p-6 md:p-8 overflow-y-auto'}`}>
          {renderContent()}
        </main>
      </div>
    </div>
